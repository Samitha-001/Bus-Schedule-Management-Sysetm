...
class Passengerprofile
{
  ...
  function api_add_friend() {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      // Process the request data and perform the update
      $friends = new Friends();
      $passenger = $_SESSION['USER']->username;
      $friend = $postData['friend'];

      $friends->addFriend($passenger, $friend);
      // Send a response
      $response = array('status' => 'success', 'data' => $postData);
      header('Content-Type: application/json');
      echo json_encode($response);
  ...
  function api_remove_friend() {
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      $friends->removeFriend($passenger, $friend);
      // Send a response
      $response = array('status' => 'success', 'data' => $postData);
      header('Content-Type: application/json');
      echo json_encode($response);
