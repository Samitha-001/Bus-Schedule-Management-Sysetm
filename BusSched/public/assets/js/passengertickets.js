document.addEventListener("DOMContentLoaded", function () {
        ...
        document.getElementById("ticket-details-booking-time").innerHTML = ticket['ticket']["booking_time"];
                ...
                halts.then((halts) => {
                    halts.forEach((halt) => {
                        ...
                        getLocationUpdates(document.getElementById("ticket-details-trip").innerHTML)
    ...
    updateLocationConfirmBtn.addEventListener("click", function () {
        ...
        let card = document.querySelector(".location-update-card.selected-halt");
        card.classList.add("halt-passed");
        card.classList.remove("selected-halt");
    ...
    //function to get location updates of user and disable last updated location buttons
    function getLocationUpdates(tripID) {
        console.log(tripID);
        let data = {tripID: tripID};
        let url = `${ROOT}/passengertickets/api_get_location_updates`;
        let options = {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
        ...
        return fetch(url, options)
            .then((response) => response.json())
            .then((data) => {
                let halts = data.data.map((item) => item.halt);
                console.log(halts);
                document.querySelectorAll('.location-update-card').forEach((item) => {
                    if (halts.includes(item.getAttribute('id'))) {
                        item.classList.add('halt-passed');
                    }
                })
                // return data.data;
            })
